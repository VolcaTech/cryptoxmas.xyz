  ...
  trust: {
    ...
    mobile: {
      android: {
        support: true,
        deepLink: url =>
          `https://links.trustwalletapp.com/a/key_live_lfvIpVeI9TFWxPCqwU8rZnogFqhnzs4D?&event=openURL&url=${encodeURIComponent(
            url
          )}`
      },
      ios: {
  ...
  opera_beta: {
    ...
    mobile: {
      android: {
        support: true,
        deepLink: url =>
          `intent://${_withoutProtocol(
            url
          )}/#Intent;scheme=http;package=com.opera.browser.beta;end`
      },
      ios: {
        support: false,
        deepLink: null
  ...
  token_pocket: {
    ...
    mobile: {
      ...
      ios: {
        support: true,
        deepLink: url =>
          `https://tokenpocket.github.io/applink?dappUrl=${encodeURIComponent(
            url
          )}`
      }
    }
